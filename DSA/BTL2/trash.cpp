...
private:
	...
	public:
		...
		unsigned long long DFS(Node* node){
			...
			int n = CountNode(node->right) + x;
			return (combinationFomula(n, x) * DFS(node->left) * DFS(node->right))%MAXSIZE;
		}
		...
		void destruct(Node* node){
			if(node == nullptr) return;
			destruct(node->left);
			if(node->right != nullptr) destruct(node->right);
			delete node;
		}
		~Tree_BST(){
			destruct(root);
...
public:
	void handloop(Node* node){
		if(node == nullptr) return;
		handloop(node->left);
		if(node->data == '\0') cout<<node->freq<<endl;
	...
	void destruct(Node* node){
		if(node == nullptr) return;
		if(node->left != nullptr) destruct(node->left);
		if(node->right != nullptr) destruct(node->right);
		delete node;
	}
	~HuffTree_AVL(){
...
public:
	void LAPSE(string name)	{
		int result = cusnew.mainCaculate(name);
		if(result == -1) return;
